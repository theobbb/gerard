...
<script lang="ts">
	...
	async function fetch_books() {
		books = await pocketbase.collection('books').getFullList();
		books.forEach((b, i) => {
...
<div class="fixed top-4 right-4 z-100 bg-bg opacity-50">
	<div class="border px-3 py-2">
		{#each books as b}
			...
			>
				<div class="block size-4" style="background-color: {b.color}"></div>
				<div>{b.title}</div>
			</button>
		{/each}
	</div>
...
>
	{#each items as { title, id, image, aspect_ratio, sort_order, book }, i}
		<a
			class={[
				'hover:opacity-100',
		...
		{#if folio}
			...
			{/if}
		{:else}
			<div class="-ml-3 space-y-3 text-xs/3.5">
				<div class="block size-4" style="background-color: {books_map[book]?.color}"></div>
				<div>{sort_order}</div>
				<div>{title}</div>
			</div>
		{/if}
